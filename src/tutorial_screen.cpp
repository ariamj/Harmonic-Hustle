...
bool Tutorial::handle_step(float elapsed_ms_since_last_update, float current_speed) {
    ...
    createText("Tutorial", vec2(gameInfo.width / 2.f, gameInfo.height / 20.f), 1.3f, Colour::white, glm::mat4(1.f), Screen::TUTORIAL, true );
    ...
    createText("Press space to continue", vec2(gameInfo.width/2.f, gameInfo.height * 7.5f / 8.f), 0.9f, Colour::white, glm::mat4(1.f), Screen::TUTORIAL, true);
    return true;
}
...
void Tutorial::renderIntroText() {
    ...
    createText("lvl 1", spriteExPos, 0.5f, Colour::green, glm::mat4(1.f), Screen::TUTORIAL, true );

    vec2 enemyExPostion = vec2(gameInfo.width * 1.5f / 10.f, gameInfo.height * 3.f / 6.f - PLAYER_HEIGHT / 2.f - 10.f);
    createText("lvl 1", enemyExPostion, 0.5f, Colour::white, glm::mat4(1.f), Screen::TUTORIAL, true );

    vec2 redEnemyExPosition = vec2(gameInfo.width * 8.5f / 10.f, gameInfo.height * 4.5f / 6.f - PLAYER_HEIGHT / 2.f - 10.f);
    createText("lvl 2", redEnemyExPosition, 0.5f, Colour::red, glm::mat4(1.f), Screen::TUTORIAL, true );
    ...
    createText("This is you! You're currently level 1. To win this game,",
        vec2(currXPos, currYPos), 0.5f, Colour::off_white, glm::mat4(1.f), Screen::TUTORIAL, true);
    ...
    Entity wasd = createText("W A S D",
        vec2(gameInfo.width * 2.5f / 10.f, currYPos), 0.65f, Colour::green, glm::mat4(1.f), Screen::TUTORIAL, true);

    Entity wasdRest = createText("keys to move around",
        vec2(gameInfo.width * 4.5f / 10.f, currYPos), 0.65f, Colour::off_white, glm::mat4(1.f), Screen::TUTORIAL, true);

    ///////////// right aligned
    float rightXPos = gameInfo.width * 6.f / 10.f;
    currYPos += 140.f;
    createText("On spawn, you'll be in the overworld surrounded by enemies.", 
        vec2(rightXPos, currYPos), 0.5f, Colour::off_white, glm::mat4(1.f), Screen::TUTORIAL, true);

    currYPos += 40.f;
    createText("Collide and ", 
        vec2(rightXPos - 270.f, currYPos), 0.65f, Colour::off_white, glm::mat4(1.f), Screen::TUTORIAL, true);
    createText("defeat same level enemies", 
        vec2(rightXPos + 150.f, currYPos), 0.65f, Colour::red, glm::mat4(1.f), Screen::TUTORIAL, true);
    ...
    currYPos += 170.f;
    createText("run away", 
        vec2(currXPos - 190.f, currYPos), 0.65f, Colour::red, glm::mat4(1.f), Screen::TUTORIAL, true);
    createText("from red enemies!", 
        vec2(currXPos + 100.f, currYPos), 0.65f, Colour::off_white, glm::mat4(1.f), Screen::TUTORIAL, true);
...
void Tutorial::renderBattleExplainText() {
    // createText("TESTING 2", vec2(gameInfo.width/2.f, gameInfo.height / 2.f), 2.f, Colour::white, glm::mat4(1.f), Screen::TUTORIAL, true);
    vec2 spriteExPos = {gameInfo.width * 1.5f / 10.f, gameInfo.height * 0.85f / 6.f - PLAYER_HEIGHT * 0.7f / 2.f - 7.f };
    createText("lvl 1", spriteExPos, 0.5f * 0.7f, Colour::white, glm::mat4(1.f), Screen::TUTORIAL, true );
    ...
    createText("If you hit one of these guys, you'll enter a battle!",
        vec2(rightXPos, currY), 0.5f, Colour::off_white, glm::mat4(1.f), Screen::TUTORIAL, true);

}


void Tutorial::renderAdvancingExplainText() {
    createText("TESTING 3", vec2(gameInfo.width/2.f, gameInfo.height / 2.f), 2.f, Colour::white, glm::mat4(1.f), Screen::TUTORIAL, true);
